...
export abstract class AbstractMigration {
   ...
   */
  protected async createCollectionIfNotExists(collectionName: string): Promise<void> {
  ...
  /** 删除集合（如果存在） */
  protected async dropCollectionIfExists(collectionName: string): Promise<void> {
    const collections = await this.db.listCollections({ name: collectionName }).toArray();
    if (collections.length > 0) {
  ...
  protected async dropIndexIfExists(collectionName: string, indexName: string): Promise<void> {
    const collection = this.db.collection(collectionName);
    const existingIndexes = await collection.indexes();
    const indexExists = existingIndexes.some((index) => index.name === indexName);
